...
% clc; 
clearvars -except directories directory dir_name i_dir n_dir N_dir

%% Create list of images inside user specified directory
% directory= fullfile('D:\210115 batch processing PIV' , 'cropped') ; %directory containing the images you want to analyze
...
r = cell(6,1);
%Parameter     %Setting                                     %Options
r{1,1}= 'Calibration factor, 1 for uncalibrated data';      r{1,2}=5.8258e-06;          % Calibration factor for u and v, 194nm/px@30fps
r{2,1}= 'Valid velocities [u_min; u_max; v_min; v_max]';    r{2,2}=[-20; 20; -20; 20];  % Maximum allowed velocities, for uncalibrated data: maximum displacement in pixels
...
%% 
save(fullfile(directory, [dir_name, '_' num2str(amount),'frames_result_16-8.mat']));
    %% 
clearvars -except p s r x y u v typevector directory filenames ...
    u_filt v_filt typevector_filt correlation_map ...
    directories directory dir_name i_dir N_dir n_dir
disp('DONE.')