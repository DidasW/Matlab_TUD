...
If no peak was detected, a BPF of [40,70]Hz of order 4(2*2), will be 
applied. For general algae signal, simply use:
          vx_BPF = AutoBPF_FlagSig_v2(vx_raw,Fs);
...
varargin: MUST be two variables to successfully set the default passband,
          ...
          e.g.: vx_BPF = AutoBPF_FlagSig_v2(vx_raw,Fs,30,60);
          NOTE: This sets the default pass band between 30 and 60 Hz.it is
          especially useful for handling extremely weak signal - where you 
          know exactly the frequency of the meaningful fraction, but cannot
          define a prominent peak from the spectrum.


The Number of outputs may vary. Following the order of:
...
function [signal_filt, varargout]= AutoBPF_FlagSig_v2(signal,Fs,varargin)
    ...
    min_log_p  = 0.5; % The smallest prominence to qualify peak
    init_log_p = 1.0;
