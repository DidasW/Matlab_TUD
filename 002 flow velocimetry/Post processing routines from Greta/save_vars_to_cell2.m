%Save variaftbles from experiments to cell variables, so all datasets can be
...
%----------------------------------------------------------------------
%Point forces [N]
fx{dd,1} = 1e-12.*fx1; %from pico-Newton converts to Newton
fx{dd,2} = 1e-12.*fx2;
...
Mom{dd,1} = 1e-18.*m1;
Mom{dd,2} = 1e-18.*m2;
Mom{dd,3} = Mom{dd,1}+Mom{dd,2};
% 
% for lr=1:3
% Mtot{dd,lr}  = sum(Mom{dd,lr},2);
% end
...
for kk=1:3; 
    ...
    Momdl{dd,kk}    = Mom{dd,kk}(:,2).*facdlF(dd); % Momentum at base
    ...
    GIPMom{dd,kk}   = griddedInterpolant(dataind{dd},Mom{dd,kk}(:,2),'linear');
    ...
    GIPMomdl{dd,kk} = griddedInterpolant(dataind{dd},Momdl{dd,kk},'linear');
end
...
for lr=1:2
    for kk=1:length(dataind{dd})
        ...
        kappanode = kappavec.*1e6;                     %Curvature at nodes         [1/m]
...
%         axis equal, grid on, hold off
        ...
        [Rx,Ry,N,Q,M,mde,mdf] = deal(zeros(size(flaggrid{dd}))); %RxRy x and y components of internal force; Normal/shear force [N], internal moment [Nm], distributed moment
 %       fnorm  = dot([fx{dd,lr}(kk,:)' fy{dd,lr}(kk,:)'],normvec,2);
 %       ftang  = dot([fx{dd,lr}(kk,:)' fy{dd,lr}(kk,:)'],tangvec,2);
        Rx(end) = 0; Ry(end) = 0; N(end) = 0; Q(end) = 0; M(end) = 0; mde(end) = 0; mdf(end) = 0;%Forces/moment zero at free end
        
        
        for nn=length(flaggrid{dd})-1:-1:1 %Loop backwards through nodes            
            intpx  = Rx(nn+1) + fx{dd,lr}(kk,nn)/2;
            intpy  = Ry(nn+1) + fy{dd,lr}(kk,nn)/2;
            mde(nn) = -EI(dd)*(kappanode(nn+1)-kappanode(nn))/ds ;
            mdf(nn) = -(tangvec(nn,1)*intpy-tangvec(nn,2)*intpx);
            
            Rx(nn) = Rx(nn+1) + fx{dd,lr}(kk,nn);
            Ry(nn) = Ry(nn+1) + fy{dd,lr}(kk,nn);
            N(nn)  = Rx(nn)*tangvec(nn,1) + Ry(nn)*tangvec(nn,2);
            Q(nn)  = Rx(nn)*normvec(nn,1) + Ry(nn)*normvec(nn,2);
            M(nn)  = EI(dd)*kappanode(nn);
        end
