...
for lr=1:2
    ...
    for k=1:avgbeat{dd,lr}.nperiods
        ...
        dataMom     = GIPMom{dd,lr}(xg);   %Total summed force             [N]
        ...
        avgbeat{dd,lr}.Momphi(k,:)  = interp1(dataphic,dataMom,phi0,'linear','extrap');
    ...
    avgbeat{dd,lr}.meanEFtphi   = mean(avgbeat{dd,lr}.EFtphi,1);
     avgbeat{dd,lr}.meanMomphi   = median(avgbeat{dd,lr}.Momphi,1);
...
avgbeat{dd,3}.meanMomphi   = avgbeat{dd,1}.meanMomphi+avgbeat{dd,2}.meanMomphi;%Momentum              [N]
...
for kk=1:3
   ...
   avgbeat{dd,kk}.meanMomphidl = avgbeat{dd,kk}.meanMomphi.*facdlF(dd);
