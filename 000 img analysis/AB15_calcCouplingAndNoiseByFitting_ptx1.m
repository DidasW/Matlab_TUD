...
for i_cell = 1:NoCell
    ...
    for i_flow = 1:numel(flowTypeList)
        ...
        idxIPEnough     = IPFraction_list>0.2;
        ...
        [f0_mean,f0_std,IPFraction] = getStressFreeBeatFreq(...
                                      experiment_path,'Strain','ptx1');
        ...
        for i_piezoFreq = 1:NoFreq
            ...
            IPFraction  = IPFraction_list(i_piezoFreq);
            ...
            x0   = [0.9 ,   1 ,   f0_mean,       0];
            lb   = [0.7 ,  0.1,   f0_mean-2*f0_std,     -pi];
            ub   = [1.2 ,   6 ,   f0_mean+2*f0_std,     pi ];
            % flag1
            ...
            if plotFitting
                ...
                if IPFraction > 0.2
                    ...
                    plot_fitAndExpPhasePDF(20,T_eff_1,epsilon_1,...
                        nu_1,psi_0_1,piezoFreq,BinCenters_pi,...
                        PDFPh_norm1,YangHong)
                    % flag2
                    subplot(4,NoCol,i_piezoFreq+2*NoCol)
                    plot_fitAndExpPhasePDF(20,T_eff_2,epsilon_2,...
                        nu_2,psi_0_2,piezoFreq,BinCenters_pi,...
                        PDFPh_norm2,BaoLan)
                end
            end
        end
        ...
        subtable1_IPEnough = subtable1(idxIPEnough,:);
        subtable2_IPEnough = subtable2(idxIPEnough,:);
        
        [x0_1,lb_1,ub_1] = prepareFittingMultiplePDFs_ptx1(...
                           subtable1_IPEnough);
        [x0_2,lb_2,ub_2] = prepareFittingMultiplePDFs_ptx1(...
                           subtable2_IPEnough);

        opts = optimoptions(@fmincon,'Display','iter');
        [x_fitAll_1,~,~] = fmincon(@(x) lsqFitPhaseLockPDF_multiple(...
                               x,BinCenters_pi,...
                               PDFExpListToUse1(idxIPEnough),...
                               freqList),...
                               x0_1,[],[],[],[],lb_1,ub_1,[],opts);
        [x_fitAll_2,~,~] = fmincon(@(x) lsqFitPhaseLockPDF_multiple(...
                               x,BinCenters_pi,...
                               PDFExpListToUse2(idxIPEnough),...
                               freqList),...
                               x0_2,[],[],[],[],lb_2,ub_2,[],opts);
        ...
        fitAllExpAtOnce.flag1.psi_0(idxIPEnough)  = x_fitAll_1(4:end);
        fitAllExpAtOnce.flag1.nuList(idxIPEnough) = freqList(idxIPEnough)...
                                                    - x_fitAll_1(3);
        
        fitAllExpAtOnce.flag2.T_eff  = x_fitAll_2(1);
        fitAllExpAtOnce.flag2.epsilon= x_fitAll_2(2);
        fitAllExpAtOnce.flag2.f0     = x_fitAll_2(3);
        fitAllExpAtOnce.flag2.psi_0(idxIPEnough)  = x_fitAll_2(4:end);
        fitAllExpAtOnce.flag2.nuList(idxIPEnough) = freqList(idxIPEnough)...
                                                    - x_fitAll_2(3);
