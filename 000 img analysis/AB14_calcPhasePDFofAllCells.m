...
for i_cell = 1:NoCell 
    ...
    for i_flow = 1:numel(flowTypeList)
        ...
        IPFraction_list = ones(NoDiffFreq,1);
        ...
        for i_piezoFreq = 1:NoDiffFreq
            ...
            idx_middlePart = find(t>  (t_Fend+t_start)/1000 + 0.2 &...
                                  t<  (t_Fend+t_start)/1000 + 10.2);
            ...
            IPFraction_list(i_piezoFreq) = IPFraction;
            %% 
            clearvars calculateTheoreticalPhase
        end
        if exist(fullfile(PDFFdpth,[flowType,'_PhasePDF.mat']),'file')
            save(fullfile(PDFFdpth,[flowType,'_PhasePDF.mat']),...
            '-append',...
            'freqList','t_Fstart_list','t_Fend_list', 't_start_list',...
            'NoBins','BinEdges_pi','BinCenters_pi',...
            'PhPDF_1_h_list' , 'PhPDF_2_h_list',...
            'PhPDF_1_Th_list', 'PhPDF_2_Th_list','IPFraction_list')
        else
            save(fullfile(PDFFdpth,[flowType,'_PhasePDF.mat']),...
            'freqList','t_Fstart_list','t_Fend_list', 't_start_list',...
            'NoBins','BinEdges_pi','BinCenters_pi',...
        ...
        if saveFig
            savefig(gcf,fullfile(PDFFdpth,...
                    [strain,'_',experiment,'_',flowType,'.fig']))
            print  (gcf,fullfile(PDFFdpth,...
                    [strain,'_',experiment,'_',flowType,'.png']),...
                    '-dpng','-r300');
            close(gcf)
        end
    end
