%% Folder structure
FOLDER_ROOT         = fullfile('F:','190125 mstg1');
...
for i_cell = 1:NoFile
    ...
    resultThisExp = {expDate,cellname,strain,fps,...
                     slipOrNot};
    slipTable = [slipTable;resultThisExp]; %#ok<AGROW>
end

varNames  = {'Date','CellName','Strain','FrameRate',...
             'SlipOrNot'};
slipTable.Properties.VariableNames= varNames;
...
save(fullfile(intermediateFdpth,...
     sprintf('slipTable_%s_%dTracks_%s.mat',[expDate,strain],...
     NoFile,processDate)),'slipTable');
